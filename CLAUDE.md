...
### Progress, metrics & sharing
- **Progress view** (`components/History/ProgressView.jsx`, third tab in History's toggle): reuses the already-loaded local+cloud `sessions`, computes each take's score/filler%/WPM/eye% and shows a plain improvement headline plus one recharts mini-trend per metric with a direction badge. No new data source, no scoring changes. Guards 0 and 1-take cases.
