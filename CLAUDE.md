...
### Dashboard tabs
Watch & Listen · Overview · Filler Words · Vocabulary · Pacing · Coaching. **The transcript lives inside Watch & Listen** (`components/Dashboard/Transcript.jsx`, rendered beneath the players); pass it the audio `mediaRef` and each word becomes clickable (seeks the audio to that word's `start`) with the word under the playhead highlighted as it plays. No standalone Transcript tab. **Pacing** (`tabs/PacingTab.jsx`) is a speedometer: an 80–200 WPM scale with the 120–160 band shaded and a marker at your average, plus a plain verdict, a steadiness read, and the long-pause list (no recharts). **Vocabulary** (`tabs/VocabularyTab.jsx`) explains unique-word ratio in plain terms, lists the content words you repeated 3+ times (stopwords/fillers excluded), and spells out what counts as "the same word."
