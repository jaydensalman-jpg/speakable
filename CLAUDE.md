...
**Whisper drops/mangles hesitations** (verified: "um"→"am"/"un", "uh"→"awe"; tiny.en is worse) — so `useSpeechRecognition` also counts them from the live recognizer's INTERIM hypotheses (per-result max, committed on final/restart, timestamped events) and `mergeFillerCounts` takes the per-label **max** of whisper vs interim (never the sum). Only unambiguous hesitations merge; crutch words repeat across interim updates and would over-count. **Chrome often scrubs interims too** (verified by real use), so the detector of last resort is ACOUSTIC (`detectVoicedGaps` in `lib/transcribe.js`): sustained voiced frames (RMS above adaptive threshold + low zero-crossing rate) inside ≥0.3s inter-word gaps, plus stretched junk words ("am" lasting ≥0.55s). Verified: a hum whisper omitted entirely → detected; a silent gap → not. If audio hears more hesitations than the text sources named, the surplus is reported under the label **`um/uh`**. Works for uploads too.
...
### Dashboard tabs
Watch & Listen · Overview · Filler Words · Pacing · Coaching. **The transcript lives inside Watch & Listen** (`components/Dashboard/Transcript.jsx`, rendered beneath the players) so you read along while listening — there is no standalone Transcript tab. **Pacing** (`tabs/PacingTab.jsx`) is a speedometer: an 80–200 WPM scale with the 120–160 band shaded and a marker at your average, plus a plain verdict, a steadiness read, and the long-pause list (no recharts).
...
- **Score card** (`components/Dashboard/ShareButton.jsx`): canvas-drawn PNG summary (score ring, stats, URL), no new deps, download-only, nothing leaves the device. Button sits in the Dashboard results header.
